...
public class UsersVo extends HashMap<String, String>
{
	...
	public static UsersVo changeDataPacket(DataPacket dp)
	{
		...
		uv.put("UserName", dp.getSenderName());
		uv.setHostName(dp.getSenderHost());
		uv.put("HostName", dp.getSenderHost());
		...
		if (buff.length >= 2)
		{
			...
			uv.put("Alias", buff[0]);
			uv.put("GroupName", buff[1]);
		}
		uv.setIp(dp.getIp());
		uv.put("UserIP", dp.getIp());
		return uv;
	}
